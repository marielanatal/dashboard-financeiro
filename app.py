...
if uploaded_file:
    ...
    # Extrair número do mês para ordenar
    ...
    for ano, col in zip([2024, 2025], [col1, col2]):
        dados = df[df["Ano"] == ano]
        fat_total = dados["Faturamento - Valor"].sum()
        meta_total = dados["Meta"].sum()
    ...
    fig = px.bar(
        df_plot,
        x="Mês",                     # Mantém formato C: "01 - Janeiro"
        y="Faturamento - Valor",
        color="Ano",
    ...
    # ===================== TABELA FINAL =====================
    st.subheader("📄 Tabela Pivotada por Ano")
